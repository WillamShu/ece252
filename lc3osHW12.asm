...
; the TRAP vector table
	...
	.FILL BAD_TRAP	; x2E
	.FILL BAD_TRAP	; x2F
	.FILL TRAP_INITDISP	; x30
	.FILL TRAP_WRITEPIXEL	; x31
	.FILL BAD_TRAP	; x32
	.FILL BAD_TRAP	; x33
	.FILL BAD_TRAP	; x34
	.FILL BAD_TRAP	; x35
...
;
; Assumes: R1 - X coordinate (0-127)
;          R2 - Y coordinate (0-124)
;          R3 - C (RGB color in 5:5:5 format)
; Returns: Nothing
;
TRAP_WRITEPIXEL	
   ST   R2, WritePixel_R2  ; context save
   ST   R0, WritePixel_R0

   LD  R0, WritePixel_mask ; mask off to ~valid values
   AND  R1, R1, R0
   AND  R2, R2, R0

   ; multiply row by 128 by shifting left 7 bits
   ADD  R2, R2, R2         ; shift by 1	
   ADD  R2, R2, R2         ; shift by 2	
   ADD  R2, R2, R2         ; shift by 3	
   ADD  R2, R2, R2         ; shift by 4	
   ADD  R2, R2, R2         ; shift by 5	
   ADD  R2, R2, R2         ; shift by 6	
   ADD  R2, R2, R2         ; shift by 7	

   LD   R0, GDisplay_Base  ; base address of video memory
   ADD  R0, R0, R2	   ; add row offset
   ADD  R0, R0, R1         ; add column offset
   STR  R3, R0, #0         ; write color to pixel

   LD   R0, WritePixel_R0  ; context restore
   LD   R2, WritePixel_R2
   RET

WritePixel_R0    .BLKW 1
WritePixel_R2    .BLKW 1
WritePixel_mask  .FILL x007F

; TRAP_INITDISP	
;
; Initializes graphical display to 64x64 white with gray 
; in other areas
;
; Assumes: None
; Returns: Nothing
;
TRAP_INITDISP	
   ST   R7, InitDisp_R7    ; context save
   ST   R5, InitDisp_R5 
   ST   R4, InitDisp_R4 
   ST   R3, InitDisp_R3 
   ST   R2, InitDisp_R2 
   ...
   LD   R0, GDisplay_Base  ; base address of video memory
   LD   R1, GDisplay_Size  ; 
   LD   R2, InitDisp_White ; get background color
   LD   R3, InitDisp_Gray  ; get unused area color
   LD   R4, InitDisp_gmask ; mask for 64x64 area

InitDisp_loop
   AND  R5, R4, R0         ; in 64x64 area?
   BRz InitDisp_colorW     ; if so, paint with white
   STR  R3, R0, #0         ; if not, write gray pixel
   BR   InitDisp_check
InitDisp_colorW
   STR  R2, R0, #0         ; write white pixel
InitDisp_check
   ADD  R0, R0, #1         ; increment address
   ADD  R1, R1, #-1        ; decrement count
   BRnp InitDisp_loop 

   LD   R0, InitDisp_R0    ; context restore
   LD   R1, InitDisp_R1
   LD   R2, InitDisp_R2
   LD   R3, InitDisp_R3
   LD   R4, InitDisp_R4
...
InitDisp_R1      .BLKW 1
InitDisp_R0      .BLKW 1
InitDisp_gmask   .FILL x2040
InitDisp_White   .FILL b0111111111111111
InitDisp_Gray    .FILL b0110001100011000

GDisplay_Base    .FILL xC000
GDisplay_Size    .FILL x3E00
